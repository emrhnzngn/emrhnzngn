...
type ClientRole = 'sender' | 'receiver';
...
type SignalMessage = {
	type: 'signal';
	code: string;
	// offer | answer | candidate | custom
	payload: unknown;
};

type OutgoingMessage =
	| { type: 'joined'; code: string; peersInRoom: number }
	| { type: 'peer-joined'; role: ClientRole }
	| { type: 'peer-left' }
	| { type: 'signal'; from: ClientRole; payload: unknown }
	| { type: 'error'; message: string };

interface SocketWithMeta extends WebSocket {
	__meta?: { code: string; role: ClientRole };
	isAlive?: boolean;
}
...
type CodeMeta = { createdAt: number; used: boolean };
const codeRegistry = new Map<string, CodeMeta>();
...
const codeToRoom = new Map<string, Set<SocketWithMeta>>();
...
function broadcastToRoom(code: string, exclude: WebSocket | null, message: OutgoingMessage) {
...
function roomPeerCount(code: string): number {
	return codeToRoom.get(code)?.size ?? 0;
}

function canJoin(code: string, role: ClientRole): true | string {
	const room = codeToRoom.get(code);
	if (!room) return true;
	if (room.size >= 2) return 'Room is full';
...
wss.on('connection', (wsRaw) => {
	const ws = wsRaw as SocketWithMeta;
	ws.isAlive = true;
	...
	ws.on('message', (data) => {
		...
		if (msg?.type === 'join') {
			const { code, role } = msg as JoinMessage;
		...
		if (msg?.type === 'signal') {
			const { code, payload } = msg as SignalMessage;
			const fromRole = (ws.__meta?.role ?? 'sender') as ClientRole;
...
const interval = setInterval(() => {
	for (const client of wss.clients as Set<SocketWithMeta>) {
